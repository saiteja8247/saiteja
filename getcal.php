...
if ($conn->connect_error) {
    ...
    $alltags=$_POST["reqr"];
    $totcal=0;
    foreach ($alltags as $index => $value) {
        $fooditem=htmlspecialchars($value);
        $sql = "Select calories from  Fooditems where item='$fooditem'";
        $stmt = $conn->query($sql);
        if($stmt!==false) {
            $cal=$stmt->fetch_assoc();
            if($cal!==null){
            $totcal=$totcal+$cal['calories'];
            }
        }
        $stmt->free();
    }
...
<body>
    ...
    <p id="totalcalres" >You are consuming approximately <?php echo "$totcal"?> calories per day.</p> 
    <div id="dailyFoodItemsContainer">
        <h3 id="ijk">Add your Daily Food Items:</h3>
        <form id="tags_form" method="post" name="tags_form" action="getcal.php">
        ...
        <div id="submit" class="submitbtn">
            <input  type="submit" style="background-color:rgb(36, 36, 225); color:white; border: none; border-radius: 5px; padding: 10px 15px; margin: 5px; cursor: pointer; transition: background-color 0.3s ease-in-out, color 0.3s ease-in-out, transform 0.3s ease-in-out;" value="Submit">
        </div>
        </form>
    </div>
    <script>
        ...
        function getCalories(){
            var all_tags=document.getElementsByClassName("fooditem");
            for(var i=0;i<all_tags.length;i++){
              console.log(all_tags[i].value);
            }
